...
local pos = parser.find_pos(f,{"is-loan", "z30-item-status","z30-call-no", "z30-barcode", "z30-collection"})
...
-- Zjistit nejvyšší signaturu. Máme 4 vadný, takže ho zjistíme ze 4. záznamu 
-- od konce
local max = sort_table[#sort_table - 4]
for i = 1, max do
	if not records[i] then
		print (string.format("F%i",i))
	end
end
...
-- 			local s = c.signatura
-- 			t[#t + 1] = s
-- 			k[s] = {c.status, c.carovy_kod, c.lokace}
-- 		end 
-- 		table.sort(t)
-- 		for _, c in ipairs(t) do
-- 			print(c, k[c][1],k[c][2], k[c][3])
-- 		end
-- 	end
-- 	used[v] = true
-- end
