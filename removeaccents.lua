...
local function prepare_table(conv, func)
	...
	local make_codes = function(base,letters)
		...
		letters:gsub('u([0-9A-F]+)', function(char)
...
-- přestalo to fungovat v novější verzi Lua
-- local codes = special_chars(prepare_table(conversion_table, tolower))
--
-- jednodušší verze. řeší jen češtinu
local ulen = unicode.utf8.len
local upos = unicode.utf8.sub
local function make_table2()
  local codes = {}
  local first = "čďěéíňóřšťúůž"
  local second = "cdeeinorstuuz"
  for i = 1, ulen(first) do
    local char= upos(first, i, i) 
    local replace = upos(second, i, i)
    -- print(char, replace)
    codes[char] = replace
  end
  return codes
end
local codes = make_table2()
